...
function draw_effective_intensity() {
  ...
  CTX.moveTo(
    intervals_centers[0],
    HEIGHT - (left_leaf_times[0] - right_leaf_times[0]) * range_power.value
  );
  for (var i = 1; i < left_leaf_times.length; i++) {
    CTX.lineTo(
      intervals_centers[i],
      HEIGHT - (left_leaf_times[i] - right_leaf_times[i]) * range_power.value
    );
  }
  CTX.stroke();
}
...
function draw_leafs() {
  ...
  for (var i = 0; i < right_leaf_times.length; i++) {
    CTX2.lineTo(
      intervals_centers[i],
      right_leaf_times[i] * TIME_SCALE + TIME_0
    );
  }
  CTX2.stroke();
}
...
function draw_effective_intensity_integral() {
  ...
  for (var i = 0; i < left_leaf_times.length; i++) {
    CTX.lineTo(
      intervals_centers[i],
      (left_leaf_times[i] - right_leaf_times[i]) * range_power.value
    );
  }
